...
typedef struct semaphoreInfo {
	...
	int oddCounter; // counter of odd threads
} SEMINFO;
...
int main(int argc, char *argv[])
{
  ...
  //you can add some suitable code anywhere in main() if required
	...
	// First find the earliest and the latest created thread to avoid deadlock
	int earlyTime = myThreads[0].startTime;
	int earlyTimeIndex = 0;
	int lateTime = myThreads[0].startTime;
	int lateTimeIndex = 0;

	for (int i = 0; i < threadCount; i++){
		if (myThreads[i].startTime >= lateTime){
			lateTime = myThreads[i].startTime;
			lateTimeIndex = i;
		}
		if (myThreads[i].startTime < earlyTime){
			earlyTime = myThreads[i].startTime;
			earlyTimeIndex = i;
		}
	}

  // initialize semaphore
	if (myThreads[earlyTimeIndex].isOdd){
		sem_init(&semInfo.semEven, 0, 1);
		sem_init(&semInfo.semOdd, 0, 0);
	} else {
		sem_init(&semInfo.semEven, 0, 0);
		sem_init(&semInfo.semOdd, 0, 1);
	}
...
void isDeadlock(){
	if (semInfo.evenCounter == 0){
		sem_post(&semInfo.semOdd);
	}

	if (semInfo.oddCounter == 0){
		sem_post(&semInfo.semEven);
	}
}
